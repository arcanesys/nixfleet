...
**Cycle state:** Phase 3 is merging. Phase 4 is the **closing phase** of the
core hardening cycle. Every open item in this file is Phase 4 scope unless
it is explicitly out-of-cycle (external dependency, separate repository).
When Phase 4 ships, this file should be empty apart from the two explicitly
out-of-cycle items under **Infrastructure / dependencies**.
...
### Agent testability refactor (in scope for Phase 4)

- [ ] **Agent poll-loop lib extraction.** The `poll_hint` honouring logic lives in `agent/src/main.rs` (around the `PollOutcome::Success { poll_hint: Some(hint) }` match arm, ~lines 170–186 at PR #30). Testing whether the real agent honours `poll_hint` requires observing its poll cadence, which needs either fake time (`tokio::time::pause()`, which does not work across OS processes) or extracting the loop into `pub async fn agent::run_loop(...)` in a new `agent/src/lib.rs` so an in-process `#[tokio::test]` can drive it deterministically.

  CP-side emission of `poll_hint` is already covered by `control-plane/tests/polling_scenarios.rs` (P1, P2). Phase 4 closes the agent half of that contract. Scope: extract the loop into `agent/src/lib.rs`, turn `agent/src/main.rs` into a thin `async fn main` that calls it, add in-process `#[tokio::test]` scenarios that assert (a) the agent schedules its next poll at `poll_hint` seconds when the CP sends a hint during an active rollout, (b) the agent reverts to the configured `pollInterval` when the hint clears. **Note:** spec § 6 R1 originally forbade testability refactors inside the hardening cycle to keep earlier phases focused; that constraint is explicitly lifted for Phase 4 because Phase 4 is the closing phase and the gap is load-bearing for the rollout polling contract.

### Test infrastructure (decide in Phase 4)

- [ ] **nixosTest shared-store makes store-path assertions tautological.** Multiple scenario tests would benefit from being able to assert "path X is on node A but not on node B" (e.g. to prove `nix copy` actually transferred something vs. was a no-op). Today that class of assertion is impossible because `/nix/store` is shared read-only via 9p across all VMs. Phase 3 workarounds were (a) VM-local marker files under `/tmp`, (b) Nix-DB queries via `nix-store -q --references` (DB is per-VM, store files are shared). A stronger approach would be per-VM store isolation via `virtualisation.useNixStoreImage = true` + `virtualisation.mountHostNixStore = false`, but that blows up build time because every VM rebuilds its own image. Phase 4 decision: either adopt the per-VM store image approach where the assertion class really matters, or close this as WONTFIX with a rationale in the docs. Do not leave it open past cycle close.

## Out-of-cycle (Phase 4 does NOT absorb)

- [ ] **#22: Revert `attic` input to upstream** when https://github.com/zhaofengli/attic/pull/300 is merged. External dependency — cannot be fixed in this repo until upstream lands.
- [ ] **Cosmetic:** Generation count fix in compliance probes (count the active system as generation 1, not 0). Lives in the `nixfleet-compliance` repository, not this one.
