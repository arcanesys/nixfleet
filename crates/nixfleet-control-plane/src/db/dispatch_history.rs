...
mod tests {
    ...
    fn mark_terminal_for_rollout_host_idempotent() {
        ...
        let n = db
            .dispatch_history()
            .mark_terminal_for_rollout_host("stable@r1", "host-02", TerminalState::RolledBack, now)
            .unwrap();
        assert_eq!(n, 1);
        let n = db
            .dispatch_history()
    ...
    fn mark_rollout_converged_skips_terminal_rows() {
        ...
        let deadline = Utc::now() + chrono::Duration::seconds(120);
        db.host_dispatch_state()
            .record_dispatch(&dispatch_insert("host-01", "stable@r1", "system", deadline))
            .unwrap();
        db.dispatch_history()
