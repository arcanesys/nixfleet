//! mTLS defense-in-depth: CN validation against the request path.
//!
//! Without this, any client cert signed by the fleet CA can be used for
//! any agent identity. The CA boundary already enforces "must be a
//! fleet member"; CN validation enforces "must be the fleet member you
//! claim to be" (via the `{id}` path segment on `/api/v1/machines/{id}/*`
//! routes).
//!
...
//! ## Wiring
//!
//! `control-plane/src/main.rs` builds the `RustlsAcceptor`, wraps it in
//! `MtlsAcceptor::new(...)`, and calls `axum_server::bind(addr).acceptor(mtls)`.
//! The `MtlsAcceptor` injects `PeerCertificates` into every request
//! extension on a connection. The `cn_matches_path_machine_id`
//! middleware (wired in `lib.rs::build_app` on agent-facing routes only)
//! reads the extension and rejects with 403 if the CN does not match
//! the `{id}` path segment.
//!
//! When mTLS is not configured (`tls.client_ca` is None), the
//! `PeerCertificates` extension still exists but is empty
//! (`is_present() == false`). The middleware treats that as a no-op
//! and lets the request through, so existing tests and dev setups
//! without client certs continue to work.
...
/// Client certificate chain extracted from the TLS connection, injected
/// into every request as an extension by [`MtlsAcceptor`]. If the client
/// did not present a certificate the chain is empty.
...
impl PeerCertificates {
    /// Construct from a chain (leaf first).
    pub fn new(chain: Vec<CertificateDer<'static>>) -> Self {
        Self { chain }
    }

    /// Construct an empty instance - the no-mTLS / no-cert case.
    pub fn empty() -> Self {
        Self { chain: Vec::new() }
    }

    /// True iff the client presented at least one certificate.
    ...
    /// Returns None if no certificate is present or the CN cannot be
    /// parsed.
...
// =====================================================================
// MtlsAcceptor - wraps RustlsAcceptor and injects peer certs
// =====================================================================
...
/// on that connection via a per-connection [`PeerCertService`] wrapper.
///
/// Built from an existing `RustlsAcceptor` so the operator's TLS config
/// (cert, key, optional client CA) flows through unchanged.
#[derive(Clone, Debug)]
    ...
    S: Send + 'static,
{
    ...
    fn accept(&self, stream: I, service: S) -> Self::Future {
        ...
        Box::pin(async move {
            ...
            // exposes peer_certificates() returning the client's cert
            // chain (None if mTLS not configured or no cert presented).
...
/// Middleware applied to agent-facing routes. Extracts the
/// [`PeerCertificates`] injected by [`MtlsAcceptor`], reads the leaf
/// CN, and rejects with 403 if it does not match the `{id}` path
/// segment.
///
/// No-op when:
/// - The request has no `PeerCertificates` extension at all (e.g. the
///   integration test harness uses raw `axum::serve` over a TCP
///   listener with no TLS layer).
/// - The `PeerCertificates` extension is present but empty (mTLS is
///   not configured at the server level).
///
/// Both no-op cases let the request through unchanged so existing
/// tests and dev setups without client certs continue to work.
pub async fn cn_matches_path_machine_id(
    Path(params): Path<HashMap<String, String>>,
    request: Request,
    next: Next,
) -> Result<Response, StatusCode> {
    // No path id → not an agent route, let it through.
    let Some(id) = params.get("id") else {
        return Ok(next.run(request).await);
    };

    // No PeerCertificates extension at all (no MtlsAcceptor in the
    // stack - e.g. the integration test harness). Let it through.
    let Some(certs) = request.extensions().get::<PeerCertificates>() else {
        return Ok(next.run(request).await);
    };

    // Empty chain (mTLS not configured at the TLS layer). Let it through.
    if !certs.is_present() {
        return Ok(next.run(request).await);
    }
