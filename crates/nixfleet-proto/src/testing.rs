...
mod tests {
    ...
    fn host_creates_channel_and_policy() {
        let f = FleetBuilder::new().host("lab", "edge").build();
        assert!(f.hosts.contains_key("lab"));
        assert!(f.channels.contains_key("edge"));
        assert!(f.rollout_policies.contains_key("p"));
        assert_eq!(f.hosts["lab"].closure_hash.as_deref(), Some("lab-closure"));
        assert_eq!(f.hosts["lab"].system, "x86_64-linux");
    }
    ...
    fn host_setters_mutate_existing() {
        let f = FleetBuilder::new()
            ...
            .host_no_closure("lab")
            .build();
        assert_eq!(f.hosts["lab"].tags, vec!["server".to_string()]);
        assert!(f.hosts["lab"].closure_hash.is_none());
    }
}
