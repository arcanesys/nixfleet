...
pub async fn wait_for_completion(
    ...
    loop {
        ...
        if rollout.status == RolloutStatus::Paused {
            ...
            let failed_batch = rollout
                .batches
                .iter()
                .find(|b| matches!(b.status, nixfleet_types::rollout::BatchStatus::Failed));
            let detail = if let Some(batch) = failed_batch {
                let unhealthy = batch
                    .machine_health
                    .values()
                    .filter(|h| {
                        matches!(h, nixfleet_types::rollout::MachineHealthStatus::Unhealthy(_))
                    })
                    .count();
                format!(
                    "batch {} health check failed ({}/{} unhealthy)",
                    batch.batch_index, unhealthy, batch.machine_ids.len()
                )
            } else {
            ...
            println!("Rollout {} paused: {}", id, detail);
            println!("  Resume with:  nixfleet rollout resume {}", id);
            println!("  Monitor with: nixfleet rollout status {} --watch", id);
            bail!("Rollout paused");
        }
