...
# seeded into the builder node's closure at nixosTest build time.
{
  ...
  # Cache signing key placeholder. We don't read it back into eval (that
  # would require IFD); we install it on the cache node at activation time
  # via a runtime `nix-store --generate-binary-cache-key` call. The
  # matching public key is irrelevant to the test because the substitution
  # step uses `--no-check-sigs`. The advertised publicKey below is a
  # syntactically valid placeholder so the cache client module accepts it.
      ...
      extraModules = [
        {
          ...
          services.nixfleet-cache-server = {
            ...
            port = 5000;
            openFirewall = true;
            signingKeyFile = "/var/lib/nixfleet-cache/signing.secret";
          };
