...
**Fix**: Bump fleet's `nixfleet` input past the fix, redeploy lab.
...
**Fix**: Bump fleet's `nixfleet` input past the fix, redeploy lab once. After that the cascade is permanently broken.
...
**Fix**: One activation cycle naturally converges (lab activates to the artifact-declared closure, which then matches on the next checkin). Not a bug; an artifact of the self-referential design. Tracked but not actively fixed - decoupling the artifact path from `inputs.self` is a possible future change.
